...
const RepositoryCard = ({ data }) => {
  const handleOpenGithub = () => {
    // Note: In a real app, you'd import and use Linking from 'react-native'
    console.log(`Attempting to open GitHub for: ${data.owner}/${data.name}`);
    // Using a console log/alert substitute since we can't use alert()
    alert(`Pretending to open link for ${data.owner}/${data.name}`);
  };
...
const styles = StyleSheet.create({
  ...
  content: {
    flex: 1,
    backgroundColor: '#e6f7ff', // Light blue/cyan background matching the image
    padding: 16,
  },
  card: {
    backgroundColor: '#fff',
